...
let codegen_rule_bodies = (rules: list(B.rule)): list((B.rule, string)) => {
   let rules_sorted_by_type = List.sort(sort_by_type, rules);
   List.map(((name, body): B.rule) => {
      ((name, body), wrap_ident(name) ++ " =" ++ codegen_rule_body(body))
...
let codegen = ((program_name, rules): B.grammar): (list(B.rule), string) => {
  counter := 0;
  ...
  let type_constructors = codegen_rule_constructor(List.map(fst, rule_name_and_types));
  let final = "type " ++ program_name ++ "_cst" ++ " = list(cst_node)";
  (rule_name_and_types, code_header ++ type_def ++ type_constructors ++ final);
}
