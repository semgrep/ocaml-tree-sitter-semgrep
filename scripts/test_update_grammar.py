...
class VendorRuntimeHeadersTests(FilesystemTest):
    SG = Path("lang") / "semgrep-grammars" / "src"

    def _make_upstream(self, lang: str, subpath: tuple[str, ...] = ()) -> Path:
        """Create an upstream tree-sitter-<lang>/<sub>/src/tree_sitter/ dir."""
        path = self.root / self.SG / f"tree-sitter-{lang}"
        for part in subpath:
            path /= part
        (path / "src" / "tree_sitter").mkdir(parents=True)
        (path / "grammar.js").write_text("// upstream grammar")
        return path

    def _make_wrapper(self, list_name: str, subpath: tuple[str, ...] = ()) -> Path:
        """Create a semgrep-<list_name>/<sub>/ with a grammar.js."""
        path = self.root / self.SG / f"semgrep-{list_name}"
        for part in subpath:
            path /= part
        path.mkdir(parents=True)
        (path / "grammar.js").write_text("// wrapper grammar")
        return path

    def test_copies_missing_runtime_headers(self):
        upstream = self._make_upstream("python")
        (upstream / "src" / "tree_sitter" / "array.h").write_text("// array")
        (upstream / "src" / "tree_sitter" / "alloc.h").write_text("// alloc")
        wrapper = self._make_wrapper("python")
        ug.vendor_runtime_headers(upstream, self.root, "python")
        self.assertEqual(
            (wrapper / "src" / "tree_sitter" / "array.h").read_text(), "// array",
        )
        self.assertEqual(
            (wrapper / "src" / "tree_sitter" / "alloc.h").read_text(), "// alloc",
        )

    def test_preserves_wrapper_parser_h(self):
        # parser.h is wrapper-specific (the wrapper has its own grammar.js).
        upstream = self._make_upstream("python")
        (upstream / "src" / "tree_sitter" / "parser.h").write_text("UPSTREAM")
        wrapper = self._make_wrapper("python")
        wrapper_inc = wrapper / "src" / "tree_sitter"
        wrapper_inc.mkdir(parents=True)
        (wrapper_inc / "parser.h").write_text("WRAPPER-SPECIFIC")
        ug.vendor_runtime_headers(upstream, self.root, "python")
        self.assertEqual((wrapper_inc / "parser.h").read_text(), "WRAPPER-SPECIFIC")

    def test_overwrites_stale_header(self):
        upstream = self._make_upstream("python")
        (upstream / "src" / "tree_sitter" / "array.h").write_text("NEW")
        wrapper = self._make_wrapper("python")
        wrapper_inc = wrapper / "src" / "tree_sitter"
        wrapper_inc.mkdir(parents=True)
        (wrapper_inc / "array.h").write_text("STALE")
        ug.vendor_runtime_headers(upstream, self.root, "python")
        self.assertEqual((wrapper_inc / "array.h").read_text(), "NEW")

    def test_idempotent_when_already_synced(self):
        upstream = self._make_upstream("python")
        (upstream / "src" / "tree_sitter" / "array.h").write_text("HEADER")
        wrapper = self._make_wrapper("python")
        wrapper_inc = wrapper / "src" / "tree_sitter"
        wrapper_inc.mkdir(parents=True)
        (wrapper_inc / "array.h").write_text("HEADER")
        before = (wrapper_inc / "array.h").stat().st_mtime_ns
        ug.vendor_runtime_headers(upstream, self.root, "python")
        # Identical content => not rewritten (mtime unchanged).
        self.assertEqual((wrapper_inc / "array.h").stat().st_mtime_ns, before)

    def test_handles_nested_grammars(self):
        # typescript-style layout: typescript/ and tsx/ subdirs in both
        # the wrapper and upstream, each with its own src/tree_sitter/.
        upstream = self._make_upstream("typescript", ("typescript",))
        (upstream / "src" / "tree_sitter" / "array.h").write_text("// ts array")
        upstream_tsx = self._make_upstream("typescript", ("tsx",))
        (upstream_tsx / "src" / "tree_sitter" / "array.h").write_text("// tsx array")
        wrapper_ts = self._make_wrapper("typescript", ("typescript",))
        wrapper_tsx = self._make_wrapper("typescript", ("tsx",))
        upstream_root = self.root / self.SG / "tree-sitter-typescript"
        ug.vendor_runtime_headers(upstream_root, self.root, "typescript")
        self.assertEqual(
            (wrapper_ts / "src" / "tree_sitter" / "array.h").read_text(),
            "// ts array",
        )
        self.assertEqual(
            (wrapper_tsx / "src" / "tree_sitter" / "array.h").read_text(),
            "// tsx array",
        )

    def test_skips_when_upstream_has_no_tree_sitter_dir(self):
        upstream = self._make_upstream("python")
        # Remove the auto-created tree_sitter dir to model a grammar with
        # no vendored runtime headers.
        (upstream / "src" / "tree_sitter").rmdir()
        wrapper = self._make_wrapper("python")
        # Should be a silent no-op.
        ug.vendor_runtime_headers(upstream, self.root, "python")
        self.assertFalse((wrapper / "src").exists())

    def test_skips_when_wrapper_grammar_missing(self):
        # Upstream has a nested grammar that the wrapper does not mirror.
        upstream = self._make_upstream("python", ("extra",))
        (upstream / "src" / "tree_sitter" / "array.h").write_text("// extra")
        self._make_wrapper("python")  # no `extra/` subdir
        upstream_root = self.root / self.SG / "tree-sitter-python"
        ug.vendor_runtime_headers(upstream_root, self.root, "python")
        # No wrapper-side `extra/` dir should have been created.
        self.assertFalse(
            (self.root / self.SG / "semgrep-python" / "extra").exists()
        )
